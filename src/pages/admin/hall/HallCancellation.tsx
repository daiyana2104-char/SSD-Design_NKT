...
export function HallCancellation() {
  ...
  const handleConfirm = () => {
    ...
    const refundable = Math.max(0, (deleteTarget.paidAmount ?? 0) - deduction);

    // mark booking cancelled
    setBookings(prev => prev.map(b => b.id === deleteTarget.id ? { ...b, status: 'Cancelled', bookingStatus: 'Cancelled', paymentStatus: refundable > 0 ? 'Refunded' : b.paymentStatus } : b));
  ...
  return (
    <div>
      ...
      <ConfirmModal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleConfirm} title="Cancel Booking" message={`Cancel booking ${deleteTarget?.bookingRef ?? ''}? Refund amount: ${Math.max(0, (deleteTarget?.paidAmount ?? 0) - deduction)}`} confirmLabel="Cancel Booking" cancelLabel="Keep" variant="danger" />
    </div>
  );
}
