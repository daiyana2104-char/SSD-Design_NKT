...
function MealBookingManagementScreen() {
  ...
  const [search, setSearch] = useState(''); const [page, setPage] = useState(1); const [modalOpen, setModalOpen] = useState(false); const [viewing, setViewing] = useState<MealBooking | null>(null); const [editing, setEditing] = useState<MealBooking | null>(null); const [paymentBooking, setPaymentBooking] = useState<MealBooking | null>(null); const [paymentAmount, setPaymentAmount] = useState('');
  ...
  const changeStatus = (booking: MealBooking, bookingStatus: BookingStatus) => { if (bookingStatus === 'Confirmed') { setEditing(booking); setViewing(null); setForm({ ...booking, bookingStatus }); setModalOpen(true); return; } persist(data.map((item) => item.id === booking.id ? { ...item, bookingStatus } : item)); toast.success(`Booking ${bookingStatus.toLowerCase()}`); };
  const addPayment = () => { if (!paymentBooking) return; const amount = Number(paymentAmount); if (!Number.isFinite(amount) || amount <= 0 || amount > paymentBooking.totalAmount - paymentBooking.amountPaid) return toast.error('Validation Error', 'Enter an amount within the outstanding balance.'); const paid = paymentBooking.amountPaid + amount; const status: PaymentStatus = paid >= paymentBooking.totalAmount ? 'Paid' : 'Partially Paid'; persist(data.map((item) => item.id === paymentBooking.id ? { ...item, amountPaid: paid, paymentStatus: status } : item)); toast.success('Payment recorded'); setPaymentBooking(null); setPaymentAmount(''); };
  const filtered = data.filter((item) => !search.trim() || item.reference.toLowerCase().includes(search.toLowerCase()) || customers.find((customerItem) => customerItem.id === item.customerId)?.name.toLowerCase().includes(search.toLowerCase())); const pageSize = 8; const totalPages = Math.max(1, Math.ceil(filtered.length / pageSize)); const paged = filtered.slice((page - 1) * pageSize, page * pageSize);
  const customerName = (id: string) => customers.find((item) => item.id === id)?.name ?? 'Unknown'; const packageName = (id: string) => packages.find((item) => item.id === id)?.name ?? 'Unknown';
  const columns: Column<MealBooking>[] = [
    { key: 'reference', header: 'Booking Reference' }, { key: 'customer', header: 'Customer', render: (item) => customerName(item.customerId) }, { key: 'eventDate', header: 'Event Date' }, { key: 'package', header: 'Meal Package', render: (item) => packageName(item.packageId) }, { key: 'mealType', header: 'Meal Type' }, { key: 'adultPax', header: 'Adult Pax', align: 'right' }, { key: 'childPax', header: 'Child Pax', align: 'right' }, { key: 'totalPax', header: 'Total Pax', align: 'right' }, { key: 'totalAmount', header: 'Total Amount', align: 'right', render: (item) => `S$${item.totalAmount.toFixed(2)}` }, { key: 'paymentStatus', header: 'Payment Status', render: (item) => <StatusBadge status={item.paymentStatus} /> }, { key: 'bookingStatus', header: 'Booking Status', render: (item) => <StatusBadge status={item.bookingStatus} /> },
    { key: 'actions', header: 'Actions', align: 'center', render: (item) => <div className="flex justify-center gap-1"><button type="button" onClick={() => openView(item)} className="rounded p-1.5 text-brown-500 hover:bg-cream-100" title="View"><Eye className="h-4 w-4" /></button><button type="button" onClick={() => openEdit(item)} className="rounded p-1.5 text-brown-500 hover:bg-cream-100" title="Edit"><Edit className="h-4 w-4" /></button><button type="button" onClick={() => setPaymentBooking(item)} className="rounded p-1.5 text-brown-500 hover:bg-cream-100" title="Payment"><CreditCard className="h-4 w-4" /></button>{item.bookingStatus === 'Confirmed' && <button type="button" onClick={() => changeStatus(item, 'Cancelled')} className="rounded p-1.5 text-brown-500 hover:bg-cream-100" title="Cancel"><X className="h-4 w-4" /></button>}{item.bookingStatus === 'Confirmed' && <button type="button" onClick={() => changeStatus(item, 'Completed')} className="rounded p-1.5 text-brown-500 hover:bg-cream-100" title="Complete"><Check className="h-4 w-4" /></button>}</div> },
  ];
  return <div><PageHeader title="Meal Booking Management" description="Create and manage meal bookings" actions={<button type="button" className="btn-primary" onClick={openCreate}><Plus className="h-4 w-4" />Add Booking</button>} /><div className="card p-4"><SearchFilterBar search={search} onSearch={(value) => { setSearch(value); setPage(1); }} searchPlaceholder="Search booking reference or customer..." filters={[]} /></div><div className="card mt-4"><DataTable columns={columns} data={paged} /><Pagination page={page} totalPages={totalPages} onPage={setPage} totalItems={filtered.length} pageSize={pageSize} /></div><Modal open={modalOpen} onClose={() => setModalOpen(false)} title={viewing ? 'View Meal Booking' : editing ? 'Edit Meal Booking' : 'Add Meal Booking'} size="xl" footer={<><button type="button" className="btn-outline" onClick={() => setModalOpen(false)}>Close</button>{!viewing && <button type="button" className="btn-primary" onClick={save}>Save</button>}</>}><div className="grid gap-4 sm:grid-cols-2"><FormField label="Booking Reference" required><TextInput disabled={!!viewing} value={form.reference} onChange={(e) => setForm({ ...form, reference: e.target.value })} /></FormField><FormField label="Event Date" required><TextInput disabled={!!viewing} type="date" value={form.eventDate} onChange={(e) => setForm({ ...form, eventDate: e.target.value })} /></FormField><FormField label="Customer" required><div className={viewing ? 'pointer-events-none opacity-60' : undefined}><Dropdown value={form.customerId} onChange={(value) => setForm({ ...form, customerId: value })} options={customers.filter((item) => item.status === 'Active').map((item) => ({ label: `${item.name} (${item.mobile})`, value: item.id }))} placeholder="Select customer" /></div></FormField>{customer && <div className="rounded-lg bg-cream-50 p-3 text-sm text-brown-600"><p className="font-medium text-brown-800">{customer.name}</p><p>{customer.mobile}</p><p>{customer.email}</p></div>}<FormField label="Meal Category" required><div className={viewing ? 'pointer-events-none opacity-60' : undefined}><Dropdown value={form.categoryId} onChange={(value) => setForm({ ...form, categoryId: value, packageId: '' })} options={activeCategories.map((item) => ({ label: item.name, value: item.id }))} placeholder="Select category" /></div></FormField><FormField label="Meal Type" required><div className={viewing ? 'pointer-events-none opacity-60' : undefined}><Dropdown value={form.mealType} onChange={(value) => setForm({ ...form, mealType: value as MealType, packageId: '' })} options={mealTypeOptions} /></div></FormField><FormField label="Meal Package" required><div className={viewing ? 'pointer-events-none opacity-60' : undefined}><Dropdown value={form.packageId} onChange={(value) => setForm(calculate({ packageId: value }))} options={(viewing ? packages.filter((item) => item.id === form.packageId) : matchingPackages).map((item) => ({ label: `${item.name} (S$${item.pricePerPax.toFixed(2)}/pax)`, value: item.id }))} placeholder="Select package" /></div></FormField><FormField label="Adult Pax" required><TextInput disabled={!!viewing} type="number" min="0" value={String(form.adultPax)} onChange={(e) => setForm(calculate({ adultPax: Number(e.target.value) }))} /></FormField><FormField label="Child Pax" required><TextInput disabled={!!viewing} type="number" min="0" value={String(form.childPax)} onChange={(e) => setForm(calculate({ childPax: Number(e.target.value) }))} /></FormField><div className="sm:col-span-2 rounded-lg bg-cream-50 p-4 text-sm text-brown-700"><div className="flex justify-between"><span>Total Pax</span><strong>{form.totalPax}</strong></div><div className="flex justify-between"><span>Package Amount</span><strong>S${form.packageAmount.toFixed(2)}</strong></div><div className="flex justify-between"><span>GST</span><strong>S${form.gst.toFixed(2)}</strong></div><div className="mt-2 flex justify-between border-t border-brown-100 pt-2 text-base text-brown-900"><span>Grand Total</span><strong>S${form.totalAmount.toFixed(2)}</strong></div></div><FormField label="Booking Status"><div className={viewing ? 'pointer-events-none opacity-60' : undefined}><Dropdown value={form.bookingStatus} onChange={(value) => setForm({ ...form, bookingStatus: value as BookingStatus })} options={bookingStatusOptions} /></div></FormField><FormField label="Payment Status"><StatusBadge status={form.paymentStatus} /></FormField></div></Modal><Modal open={!!paymentBooking} onClose={() => setPaymentBooking(null)} title="Add Meal Payment" size="sm" footer={<><button type="button" className="btn-outline" onClick={() => setPaymentBooking(null)}>Cancel</button><button type="button" className="btn-primary" onClick={addPayment}>Save Payment</button></>}><FormField label="Outstanding Balance"><p className="text-sm text-brown-700">S${((paymentBooking?.totalAmount ?? 0) - (paymentBooking?.amountPaid ?? 0)).toFixed(2)}</p></FormField><FormField label="Payment Amount" required><TextInput type="number" min="0" step="0.01" value={paymentAmount} onChange={(e) => setPaymentAmount(e.target.value)} /></FormField></Modal></div>;
}
